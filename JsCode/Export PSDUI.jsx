...
function main(){
    ...
    sceneData += "</psdSize>";
    sceneData += "<moduleName>" + moduleName+ "</moduleName>";
    sceneData += "\n<layers>";
...
function getModuleName(obj)
{
    ...
    {
        for (var index = 0; index < obj.layers.length; index++) {
            var strArr = obj.layers[0].name.split("@");
            if(strArr.length > 0)
            {
                return strArr[1];
            }
...
function exportLayerSet(_layer)
{
    ...
    else if (_layer.name.search("@ScrollView") >= 0)
    {
    ...
    else
    {
        ...
        if (_layer.name.search("@") >= 0) 
        {
            tempName = _layer.name.substring(0, _layer.name.search("@"))
        }

        sceneData += "<Layer>";
        sceneData += "<type>Normal</type>";
        sceneData += "<name>" + tempName + "</name>";    
        sceneData += "<layers>";
        exportAllLayers(_layer)
        sceneData += "</layers>";
        sceneData += "</Layer>";
...
function exportPrefab(obj)
{
    ...
    {
        alert("检查 是否有 @Size 图层，@Size 图层大小与Prefab一致")
    }
    else
    {
        obj.visible = ture;
...
function exportList(obj)
{
    ...
    if (params.length < 2)
    {
        alert(obj.name + "-------检查是否参数合理 需要填:H:一排几个------------");
    }

    var recSize;
    if (obj.layers[obj.layers.length - 1].name.search("@Size") < 0)
    {
        alert("检查 是否有 @Size图层,@Size 图层大小与list大小一致");
    }
    else
    {
        obj.layers[obj.layers.length - 1].visible = true;
    ...
    sceneData += "<string>" + params[2] + "</string>"; //一行几个
    sceneData += "</arguments>";
...
{
    sceneData += "<Layer>";
    sceneData += "<type>ItemRender</type>";
    var itemName = obj.name.substring(0, obj.name.search("@"));
    sceneData += "<name>" + itemName + "</name>";    
    sceneData += "</Layer>";
}
...
function makeValidFileName(fileName)
{
    ...
    if (validName.match("Common") || 
        ...
        validName.match("Atlas") ||
        validName.match("Texture"))
    {
        validName = validName.substring (0,validName.lastIndexOf ("@"));  //截取@之前的字符串作为图片的名称。
			...
			!sourcePsdName.match("Atlas"))    // 判断是否为公用的PSD素材文件，如果不是，则自动为图片增加后缀，防止重名。 公用psd文件的图片层不允许重名。
    {
        // validName += "_" + uuid++;
    }
